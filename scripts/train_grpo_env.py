...
from env_function.goof_spiel_environment_function import (
...
from env_function.gin_rummy_environment_function import (
...
from env_function.liars_dice_environment_function import (
...
from env_function.leduc_poker_environment_function import (
...
def main():
    ...
    try:
        ...
        # Per-game max_steps routing
        if training_args.environment_name == "liars_dice":
            max_steps = 900    # short game, many rollouts per step
        elif training_args.environment_name == "gin_rummy":
            max_steps = 300    # long game, moderate rollouts
        elif training_args.environment_name == "leduc_poker":
            max_steps = 500    # medium game, boss round
        else:
            max_steps = train_request.get("max_steps", 300)
        log_info(f"max_steps: {max_steps} (game={training_args.environment_name})")
        ...
        if is_reasoning_tokenizer(tokenizer):
            ...
            if training_args.environment_name == "goof_spiel":
                ...
                training_args.initial_max_turn = 20  # reasoning model: 2× avg game (5-10 moves)
                trainer_class = ActionMaskedGRPOTrainer
            elif training_args.environment_name == "leduc_poker":
                ...
                training_args.initial_max_turn = 8   # reasoning model: Leduc max ~4 betting actions
                trainer_class = ActionMaskedGRPOTrainer
            else:
            ...
            if training_args.environment_name == "goof_spiel":
                ...
                rollout_func = liars_dice_rollout_full_prompt_and_completion_parallelized_curriculum
                reward_func = liars_dice_rollout_reward_func
                training_args.initial_max_turn = 12  # avg game 5-10 moves, 12 = safe ceiling
                trainer_class = ActionMaskedGRPOTrainer
            elif training_args.environment_name == "leduc_poker":
                rollout_func = leduc_poker_rollout_full_prompt_and_completion_parallelized_curriculum
                reward_func = leduc_poker_rollout_reward_func
                training_args.initial_max_turn = 8   # Leduc max ~4 betting actions
                trainer_class = ActionMaskedGRPOTrainer
            else:
                raise ValueError(f"Unsupported environment_name: {training_args.environment_name}")
